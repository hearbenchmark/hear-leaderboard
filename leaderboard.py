...
df_imputed_scaleavg_winsor = df_imputed_scaleavg_winsor.sort_values(
    "avgscore", ascending=False
)
...
model_corr_min = np.percentile(
    df_corr_models.to_numpy().flatten(), MODEL_CORR_PERCENTILE
)
model_corr_max = np.percentile(
    df_corr_models.to_numpy().flatten(), 100 - MODEL_CORR_PERCENTILE
)
task_corr_min = np.percentile(df_corr_tasks.to_numpy().flatten(), TASK_CORR_PERCENTILE)
task_corr_max = np.percentile(
    df_corr_tasks.to_numpy().flatten(), 100 - TASK_CORR_PERCENTILE
)
...
for r in tqdm(list(range(df_corr_tasks_dist.shape[0]))):
    ...
    for i in tqdm(range(10)):
        route_finder = RouteFinder(
            df_corr_tasks_dist, list(df_corr_tasks.index), iterations=250, verbose=False
        )
        distance, permutation = route_finder.solve()
...
df_corr_models_winsor = df_corr_models_winsor.where(
    ((df_corr_models > model_corr_max) | (df_corr_models <= model_corr_min))
)
...
sns.heatmap(
    df_corr_models_winsor,
    annot=zeroannot(df_corr_models_winsor, sigfig=2),
    annot_kws={"size": 12},
    ax=ax,
    fmt="",
    cmap=CMAP,
...
df_corr_tasks_winsor = df_corr_tasks_winsor.where(
    ((df_corr_tasks > task_corr_max) | (df_corr_tasks <= task_corr_min))
)
...
sns.heatmap(
    df_corr_tasks_winsor.astype("float"),
    annot=zeroannot(df_corr_tasks_winsor, sigfig=2),
    annot_kws={"size": 11},
    ax=ax,
    fmt="",
    cmap=CMAP,
